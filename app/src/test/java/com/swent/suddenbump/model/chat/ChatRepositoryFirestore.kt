...
class ChatRepositoryFirestoreTest {
    ...
    @Mock
    private lateinit var mockMessagesSubCollection: CollectionReference
    @Mock
    ...
    fun test_fetchMessages_success() = runBlocking {
        ...
        `when`(mockMessagesCollection.orderBy("timestamp", Query.Direction.ASCENDING)).thenReturn(mockQuery)
    ...
    fun test_getMessages_success() = runBlocking {
        ...
        `when`(mockChatDocument.collection("messages")).thenReturn(mockMessagesSubCollection)
        `when`(mockMessagesSubCollection.orderBy("timestamp", Query.Direction.DESCENDING)).thenReturn(mockQuery)
    ...
    fun test_getChatSummaries_success() = runBlocking {
        mockStatic(FirebaseAuth::class.java).use { firebaseAuthMockStatic ->
            // Arrange
            val currentUserId = "user456"
            val chatSummary1 = ChatSummary("chat1", "Hello", "user1", Timestamp.now(), 0, listOf("user1", currentUserId))
            val chatSummary2 = ChatSummary("chat2", "Hi", "user2", Timestamp.now(), 0, listOf("user2", currentUserId))
            val mockQuerySnapshot = mock(QuerySnapshot::class.java)
            val mockDocumentSnapshot1 = mock(DocumentSnapshot::class.java)
            val mockDocumentSnapshot2 = mock(DocumentSnapshot::class.java)
            val mockFirebaseUser = mock(FirebaseUser::class.java)
            ...
            // Mock Firestore query
            `when`(mockChatsCollection.whereArrayContains("participants", currentUserId)).thenReturn(mockQuery)
            doAnswer { invocation ->
                val listener = invocation.getArgument<EventListener<QuerySnapshot>>(0)
                listener.onEvent(mockQuerySnapshot, null)
                mockListenerRegistration
