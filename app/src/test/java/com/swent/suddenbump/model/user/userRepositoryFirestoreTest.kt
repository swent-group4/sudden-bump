...
class UserRepositoryFirestoreTest {
    ...
    fun deleteUserAccount_allDeletionsSuccessful_callsOnSuccess() {
        ...
        whenever(mockPhoneCollectionReference.document(phoneNumber).delete()).thenReturn(deletePhoneTask)
        whenever(mockEmailCollectionReference.document(emailAddress).delete()).thenReturn(deleteEmailTask)

        // Mock profile picture deletion
        val profilePicturesRefField = UserRepositoryFirestore::class.java.getDeclaredField("profilePicturesRef")
        profilePicturesRefField.isAccessible = true
        ...
        verify(mockUserDocumentReference).get()
        verify(mockPhoneCollectionReference.document(phoneNumber)).delete()
        verify(mockEmailCollectionReference.document(emailAddress)).delete()
        verify(mockProfilePictureRef).delete()
        verify(mockUserDocumentReference).delete()
    }
    ...
    fun deleteUserAccount_failureOnGettingUserDoc_callsOnFailure() {
        ...
        assertFalse(onSuccessCalled)
        assertTrue(onFailureCalled)
        assertEquals("User not found", capturedException?.message)
    }
    ...
    fun deleteUserAccount_failureOnDeletingPhoneDocument_callsOnFailure() {
        ...
        `when`(mockChatCollectionReference.whereArrayContains(anyString(), any())).thenReturn(mockQuery)
        `when`(mockQuery.get()).thenReturn(mockQuerySnapshot)
       // doAnswer { invocation -> }.`when`(mockQuerySnapshot).addOnSuccessListener(any(), any())
        ...
        whenever(mockPhoneCollectionReference.document(phoneNumber).delete()).thenReturn(Tasks.forException(exception))
        ...
        assertFalse(onSuccessCalled)
        assertTrue(onFailureCalled)
        assertEquals("Failed to delete phone doc", capturedException?.message)
        verify(mockPhoneCollectionReference.document(phoneNumber)).delete()
    }

    @Test
    fun deleteUserAccount_failureOnDeletingProfilePicture_callsOnFailure() {
        whenever(mockFirestore.collection("chats")).thenReturn(mockChatCollectionReference)
        ...
        whenever(mockPhoneCollectionReference.document(phoneNumber).delete()).thenReturn(Tasks.forResult(null))
        whenever(mockEmailCollectionReference.document(emailAddress).delete()).thenReturn(Tasks.forResult(null))

        // Mock profile picture deletion failure
        val profilePicturesRefField = UserRepositoryFirestore::class.java.getDeclaredField("profilePicturesRef")
        profilePicturesRefField.isAccessible = true
        val mockProfilePicturesRef = mock(StorageReference::class.java)
        profilePicturesRefField.set(userRepositoryFirestore, mockProfilePicturesRef)
        ...
        whenever(mockProfilePictureRef.delete()).thenReturn(Tasks.forException(exception))
        ...
        assertFalse(onSuccessCalled)
        assertTrue(onFailureCalled)
        assertEquals("Failed to delete profile picture", capturedException?.message)
        verify(mockProfilePictureRef).delete()
        verify(mockUserDocumentReference, never()).delete() // User doc not deleted after failure
    }
    ...
    fun deleteUserAccount_failureOnDeletingChat_callsOnFailure() {
        ...
        whenever(chatSnapshot.documents).thenReturn(listOf(chatDocSnapshot))

        // Simulate failure on deleting chat
        val exception = Exception("Failed to delete chat")
        whenever(chatDocRef.delete()).thenReturn(Tasks.forException(exception))
        ...
        assertFalse(onSuccessCalled)
        assertTrue(onFailureCalled)
        assertEquals("Failed to delete chat", capturedException?.message)
        verify(chatDocRef).delete()
        verify(mockUserDocumentReference, never()).delete() // User doc not deleted after failure
    }
