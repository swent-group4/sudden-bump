...
class DeleteUserAccountTests {
    ...
    @Mock
    private lateinit var mockPhoneDocumentReference: DocumentReference
    ...
    @Mock
    private lateinit var mockPhoneDocumentSnapshot: DocumentSnapshot

    @Mock
    private lateinit var mockPhoneQuerySnapshot: QuerySnapshot
    ...
    @Mock
    private lateinit var mockFriendDocumentReference: DocumentReference

    @Mock
    private lateinit var mockFriendDocumentSnapshot: DocumentSnapshot
    @Mock
    ...
    fun setUp() {
        ...
        `when`(mockContext.getSharedPreferences(
            anyString(),
            org.mockito.kotlin.eq(Context.MODE_PRIVATE)
        ))
        ...
        `when`(
            mockSharedPreferencesManager.saveString(
                anyString(),
                anyString()
            )
        ...
        `when`(
            mockSharedPreferencesManager.saveBoolean(
                anyString(),
                anyBoolean()
            )
        ).thenAnswer { invocation ->
        ...
        `when`(mockUserCollectionReference.document(org.mockito.kotlin.any())).thenReturn(mockUserDocumentReference)
        ...
        `when`(mockEmailCollectionReference.document(org.mockito.kotlin.any())).thenReturn(mockEmailDocumentReference)
        ...
        `when`(mockPhoneCollectionReference.document(org.mockito.kotlin.any())).thenReturn(mockPhoneDocumentReference)
    ...
    fun deleteUserAccount_allDeletionsSuccessful_callsOnSuccess() {
        ...
        whenever(mockPhoneCollectionReference.document(phoneNumber).delete()).thenReturn(
            deletePhoneTask
        )
        whenever(mockEmailCollectionReference.document(emailAddress).delete()).thenReturn(
            deleteEmailTask
        )
    ...
    fun deleteUserAccount_failureOnDeletingPhoneDocument_callsOnFailure() {
        ...
        `when`(mockChatCollectionReference.whereArrayContains(anyString(), org.mockito.kotlin.any())).thenReturn(
            mockQuery
        )
...
//        `when`(mockQuerySnapshot.isSuccessful).thenReturn(false)
        doAnswer { invocation ->
            mockQuerySnapshot
        }.`when`(mockQuerySnapshot).addOnSuccessListener(org.mockito.kotlin.any())
        doAnswer { invocation ->
            onFailureCalled = true
            capturedException = mockException
            mockQuerySnapshot
        }.`when`(mockQuerySnapshot).addOnFailureListener(org.mockito.kotlin.any())
    ...
    fun deleteUserAccount_failureOnDeletingChat_callsOnFailure() {
        ...
        whenever(mockChatDocRef.delete()).thenReturn(Tasks.forException(Exception("Failed to delete chat")))
        ...
        whenever(mockChatCollectionReference.whereArrayContains("participants", uid)).thenReturn(
            mockChatsQuery
        )
    }
