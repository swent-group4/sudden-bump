...
class NominatimLocationRepository(private val client: OkHttpClient) : LocationRepository {
   ...
   * @param onSuccess A callback invoked with the list of [Location] objects upon a successful
   *   search.
  ...
  override fun search(
    ...
    val request = Request.Builder().url(url).addHeader("User-Agent", "SuddenBump").build()
