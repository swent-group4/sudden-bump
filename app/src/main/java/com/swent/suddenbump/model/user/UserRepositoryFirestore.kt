        ...
        .addOnCompleteListener { result ->
          if (result.isSuccessful) {
            ...
            for (documentSnapshot in usersDocument) {
              runBlocking {
                try {
                  val userEmail = documentSnapshot.data!!.get("emailAddress").toString()
                  val userPhoneNumber = documentSnapshot.data!!.get("phoneNumber").toString()
                  val userFirstName = documentSnapshot.data!!.get("firstName").toString()
                  val userLastName = documentSnapshot.data!!.get("lastName").toString()
                  val userUID = documentSnapshot.id

                  if (userEmail.contains(query) ||
                      userPhoneNumber.contains(query) ||
                      userFirstName.contains(query) ||
                      userLastName.contains(query)) {
                    val path = helper.uidToProfilePicturePath(userUID, profilePicturesRef)
                    imageRepository.downloadImage(
                        path,
                        onSuccess = {
                          matchingUsers += helper.documentSnapshotToUser(documentSnapshot, it)
                        },
                        onFailure = { onFailure(it) })
                  }
                } catch (e: Exception) {
