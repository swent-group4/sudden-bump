                ...
                db.collection(usersCollectionPath)
                    ...
                    .addOnCompleteListener { resultUser ->
                        if (resultUser.isSuccessful) {
                            val path = helper.uidToProfilePicturePath(
                                resultEmail.data!!["uid"].toString(),
                                profilePicturesRef
                            )
                            imageRepository.downloadImage(path, onSuccess = {
        ...
        db.collection(usersCollectionPath)
            ...
            .addOnCompleteListener { resultUser ->
                if (resultUser.isSuccessful) {
                    ...
                    imageRepository.downloadImage(path, onSuccess = { image ->
            ...
            runBlocking {
                try {
                    ...
                    if (documentSnapshot.exists()) {
                        val path =
                            helper.uidToProfilePicturePath(uid, profilePicturesRef)
                        var profilePicture: ImageBitmap? = null
                        imageRepository.downloadImage(path, onSuccess = { pp ->
                            profilePicture = pp
                        }, onFailure = { e -> onFailure(e) })
                        val user =
