...
class UserRepositoryFirestore(
                        ...
                        for (doc in documents) {
                            ...
                            imageRepository.downloadImageAsync(
                                ...
                                onSuccess = { image ->
                                    ...
                                    val userFriend =
                                        helper.documentSnapshotToUser(doc, profilePicture)
    ...
    override fun deleteFriend(
        ...
        db.runTransaction { transaction ->
            ...
            val currentUserFriendsList =
                (currentUserData["friendsList"] as? List<String>)?.toMutableList()
                    ?: mutableListOf()
    ...
    override fun getRecommendedFriends(
        ...
        db.collection(usersCollectionPath)
            ...
            .addOnSuccessListener { userDocument ->
                ...
                val blockedList =
                    userDocument.data?.get("blockedList") as? List<String> ?: emptyList()
                        ...
                        val recommendedFriends =
                            result.documents.mapNotNull { document ->
                                ...
                                val otherUserBlockedList =
                                    document.data?.get("blockedList") as? List<String>
                                        ?: emptyList()
                                val otherUserFriendsList =
                                    document.data?.get("friendsList") as? List<String>
                                        ?: emptyList()
    ...
    override fun blockUser(
        ...
        db.runTransaction { transaction ->
            ...
            val currentUserBlockedList =
                (currentUserData["blockedList"] as? List<String>)?.toMutableList()
                    ?: mutableListOf()
            val currentUserFriendsList =
                (currentUserData["friendsList"] as? List<String>)?.toMutableList()
                    ?: mutableListOf()
            ...
            val blockedUserFriendsList =
                (blockedUserData["friendsList"] as? List<String>)?.toMutableList()
                    ?: mutableListOf()
        ...
        userRef.get().addOnCompleteListener { task ->
            ...
            Tasks.whenAllComplete(chatsQueryTask)
                .addOnSuccessListener {
                    ...
                    // Finally, Step 5: Delete the user document itself
                    ...
                    Tasks.whenAllComplete(deletionTasks).addOnCompleteListener { allTask ->
                        ...
                        if (failedTask != null) {
                            onFailure(
                                failedTask.exception ?: Exception("Unknown error during deletion")
                            )
                        } else {
                            // All tasks succeeded
                            onSuccess()
    ...
    override fun getBlockedBy(
        ...
        //go in each document in collection "users" and if uid is in their field "blockedList"
    }
        ...
        db.collection(usersCollectionPath)
            ...
            .addOnSuccessListener { result ->
                if (result.data?.get("locationSharedWith") == null) {
                    ...
                    documentSnapshotToUserList(
                        result.data?.get("locationSharedWith").toString(),
                        onSuccess = { onSuccess(it) })
                }
            }
    }
