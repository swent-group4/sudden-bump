...
class ChatRepositoryFirestore(private val firestore: FirebaseFirestore) : ChatRepository {
    ...
    override suspend fun fetchMessages(): List<Message> {
        return try {
            val querySnapshot = messagesCollection.orderBy("timestamp", Query.Direction.ASCENDING).get().await()
    ...
    override suspend fun getOrCreateChat(userId: String): String {
        ...
        try {
            val chatQuery = firestore.collection("chats").whereArrayContains("participants", userId).get().await()
    ...
    override fun getMessages(chatId: String): Flow<List<Message>> = callbackFlow {
        ...
        try {
            registration = firestore.collection("chats").document(chatId).collection("messages")
                ...
                .addSnapshotListener { snapshot, error ->
                    ...
                    val messages = snapshot?.documents?.mapNotNull { document -> document.toObject(Message::class.java) } ?: emptyList()
                    trySend(messages)
                }
        } catch (e: Exception) {
            close(e)
